...
async function flattenCoverageReports() {
  ...
  for await (const { path: filePath } of readdirp(coverageDirectory, {
    fileFilter: "lcov.info",
  })) {
    const flattenedFileName = flattenedCoverageReportFileName(filePath);

    renameSync(
      path.join(coverageDirectory, filePath),
      path.join(coverageDirectory, flattenedFileName)
    );
  }
}
